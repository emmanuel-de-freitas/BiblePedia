 ...
 */
export interface EnvironmentVariables extends
 ...
 */
export interface ServerEnvironmentVariables extends Omit<EnvironmentVariables, keyof ClientEnvironmentVariables> {
...
export class EnvironmentValidationError extends Error {
  constructor(
    ...
    const message = [
      ...
      Object.keys(invalidVariables).length > 0 && `Invalid variables: ${Object.entries(invalidVariables).map(([key, error]) => `${key} (${error})`).join(', ')}`
    ].filter(Boolean).join('\n');
