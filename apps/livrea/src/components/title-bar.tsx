	...
	if (!isMounted) {
		return (
			<div className="fixed right-0 top-0 z-[9999] flex h-[52px] w-full select-none items-center justify-between px-4">
				...
				>
					<span className="text-sm font-medium capitalize tracking-tight">Livrea</span>
				</div>
			</div>
		);
	}
	...
	return (
		<div className="fixed right-0 top-0 z-[9999] flex h-[52px] w-full select-none items-center justify-between px-4">
			...
			<div
				...
				data-tauri-drag-region
			>
				<span className="text-sm font-medium capitalize tracking-tight">Livrea | {title}</span>
			</div>
		</div>
	);
};
