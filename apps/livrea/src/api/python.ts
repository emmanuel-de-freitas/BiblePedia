...
export class PythonAPI {
	 ...
	 */
	public async extractKeywords(text: string, maxKeywords = 10): Promise<string[]> {
		try {
	 ...
	 */
	public async calculateReadingTime(text: string, wordsPerMinute = 200): Promise<ReadingTime> {
		try {
...
export const python = {
	...
	extractKeywords: (text: string, maxKeywords?: number) =>
		pythonAPI.extractKeywords(text, maxKeywords),
	calculateReadingTime: (text: string, wordsPerMinute?: number) =>
		pythonAPI.calculateReadingTime(text, wordsPerMinute),
	generateBookHash: (title: string, author: string, content?: string) =>
		pythonAPI.generateBookHash(title, author, content),
	cleanText: (text: string) => pythonAPI.cleanText(text),
	extractSentences: (text: string, maxSentences?: number) =>
		pythonAPI.extractSentences(text, maxSentences),
