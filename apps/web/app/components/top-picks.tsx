...
const TopPicks = ({ books, loading = false }: TopPicksProps) => {
	if (loading) {
		return (
			<div>
				...
				<Text textStyle={{ marginTop: 8, opacity: 0.6 }}>Loading top picks...</Text>
			</div>
		);
	}
	...
	return (
		<div>
			...
			{books.length === 0 && (
				<Text textStyle={{ marginTop: 8, opacity: 0.6 }}>No top picks available yet.</Text>
			)}
			{books.length > 0 && (
					...
					{books.map((book) => (
						...
						>
							...
							{typeof book.author === "string" && book.author && (
								<Text variant="caption" className="mt-1 text-default-500">
									{book.author}
								</Text>
							)}
						</div>
